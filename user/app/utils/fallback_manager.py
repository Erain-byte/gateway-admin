...
class FallbackManager:
    ...
    async def cache_get(self, key: str) -> Optional[str]:
        ...
        if not self.is_fallback("redis"):
            try:
                ...
                redis_instance = redis.get_instance()
    ...
    async def cache_set(self, key: str, value: str, ttl: int = 3600):
        ...
        if not self.is_fallback("redis"):
            try:
                ...
                redis = get_redis_pool()
                redis_instance = redis.get_instance()
                await redis_instance.setex(key, ttl, value)
                return
            except Exception as e:
    ...
    def is_hmac_enabled(self) -> bool:
        """检查是否启用 HMAC 验证"""
        return not self.is_fallback("gateway") and self._default_configs.get("hmac_enabled", False)
    ...
    async def try_recover_gateway(self) -> bool:
        ...
        try:
            ...
            response = await httpx_pool.get(
                f"{settings.GATEWAY_URL}/healthz",
                timeout=3.0
            )
    ...
    async def start_auto_recovery(self):
        ...
        while True:
            try:
                await asyncio.sleep(30)  # 每 30 秒尝试恢复
                
                recovered = False
                
                # 尝试恢复 Redis
                if self.is_fallback("redis"):
                    if await self.try_recover_redis():
                        recovered = True
                
                # 尝试恢复 Gateway
                if self.is_fallback("gateway"):
                    if await self.try_recover_gateway():
                        recovered = True
                
                # 如果所有服务都恢复了，停止任务
                if recovered and not self.is_fallback("redis") and not self.is_fallback("gateway"):
                    logger.info("All services recovered, stopping auto recovery")
                    break
